...
func setupTestRouter(module *Module) *gin.Engine {
	r := gin.New()
	r.GET("/download/*filepath", module.DownloadFile)
	...
	{
		auth.POST("/files/presign", module.PreSignUploadHandler)
	}
	return r
}

func TestPreSignUploadHandler(t *testing.T) {
	r := setupTestRouter(NewModule(&fakeStorageProvider{}, nil))

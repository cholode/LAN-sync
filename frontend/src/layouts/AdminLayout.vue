  ...
  <div class="admin-shell">
    ...
    <div class="main" :class="{collapsed:ui.sidebarCollapsed}">
      <header class="topbar">
        ...
        <div class="top-actions"><StatusBadge tone="success" dot>服务正常</StatusBadge><button class="icon-btn" @click="ui.toggleTheme"><Moon v-if="ui.theme==='light'" :size="17"/><Sun v-else :size="17"/></button><button class="icon-btn"><Bell :size="17"/><span class="notice"></span></button><div class="avatar">{{ initial }}</div></div>
      </header>
...
import { Users, LayoutDashboard, MessagesSquare, ShieldCheck, Bot, Activity, ScrollText, MessageSquareText, PanelLeftClose, PanelLeftOpen, Search, Moon, Sun, Bell } from 'lucide-vue-next'
import AppLogo from '../components/common/AppLogo.vue'; import StatusBadge from '../components/common/StatusBadge.vue'; import { useUiStore } from '../stores/ui.js'; import { useAuthStore } from '../stores/auth.js'
const ui=useUiStore(), auth=useAuthStore(); const initial=computed(()=>String(auth.user?.username||'A').slice(0,1).toUpperCase())
...
const ops=[{to:'/admin/system',label:'系统运行',icon:Activity},{to:'/admin/audit',label:'审计日志',icon:ScrollText}]
</script>
<style scoped>

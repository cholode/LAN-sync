  ...
  <div class="admin-shell">
    ...
    <div class="main" :class="{collapsed:ui.sidebarCollapsed}">
      <header class="topbar">
        ...
        <div class="top-actions"><button class="icon-btn" @click="ui.toggleTheme"><Moon v-if="ui.theme==='light'" :size="17"/><Sun v-else :size="17"/></button><div class="avatar">{{ initial }}</div></div>
      </header>
...
import { Users, MessagesSquare, ShieldCheck, Bot, ScrollText, MessageSquareText, PanelLeftClose, PanelLeftOpen, Search, Moon, Sun, FolderOpen } from 'lucide-vue-next'
import AppLogo from '../components/common/AppLogo.vue'; import { useUiStore } from '../stores/ui.js'; import { useAuthStore } from '../stores/auth.js'
...
const primary=[{to:'/admin/users',label:'用户封禁与权限',icon:Users},{to:'/admin/rooms',label:'群聊管理',icon:MessagesSquare}]
const ai=[{to:'/admin/moderation',label:'内容治理',icon:ShieldCheck},{to:'/admin/agent',label:'Agent & RAG',icon:Bot}]
const ops=[{to:'/admin/files',label:'文件审核与删除',icon:FolderOpen},{to:'/admin/audit',label:'审计日志',icon:ScrollText}]
</script>
<style scoped>

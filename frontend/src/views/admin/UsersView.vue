<template>
  <div class="page">
    <div class="page-head"><div><h1 class="page-title">用户封禁与权限</h1><p class="page-subtitle">封禁、解封用户，并按权限管理后台角色</p></div><button class="btn" @click="exportCsv"><Download :size="15"/>导出</button></div>
...
<script setup>
import{onMounted,ref}from'vue';import{Search,Download}from'lucide-vue-next';import StatusBadge from'../../components/common/StatusBadge.vue';import{adminApi}from'../../api/admin.js';
...
async function manage(u){const action=prompt('操作：ban / unban / role_user / role_operator / role_moderator / role_super_admin',u.status==='banned'?'unban':'ban');if(!action)return;if(!confirm(`对 ${u.username||u.id} 执行 ${action}？`))return;try{await adminApi.userAction(u.id,action.trim());await load()}catch(e){alert(e.message)}}

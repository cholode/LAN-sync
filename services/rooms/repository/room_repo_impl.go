...
func (r *roomRepoImpl) SearchRooms(keyword string, offset, limit int) ([]*roomsmodel.Room, int64, error) {
	query := r.db.Model(&roomsmodel.Room{}).Where("type = ? AND status = ?", 2, 0)
...
func (r *roomRepoImpl) CreateRoomWithCreator(room *roomsmodel.Room, creatorID int64) error {
...
func (r *roomRepoImpl) GetRoomByID(roomID int64) (*roomsmodel.Room, error) {
...
func (r *roomRepoImpl) SoftDeleteRoom(roomID int64) error {
	...
	return r.db.Delete(&roomsmodel.Room{}, roomID).Error
}

// GetJoinedRooms 联表查询用户加入的群聊，避免N+1查询问题
func (r *roomRepoImpl) GetJoinedRooms(userID int64) ([]*roomsmodel.Room, error) {
	var rooms []*roomsmodel.Room
	// 内连接查询，继承软删除规则，一次性获取用户所有群聊
	err := r.db.Model(&roomsmodel.Room{}).
		Select("rooms.*").
...
func (r *roomRepoImpl) GetRoomByExactName(exactName string) (*roomsmodel.Room, error) {
	var room roomsmodel.Room
